...
public class PlayerMovement : MonoBehaviour
{
    ...
    private void FixedUpdate(Quaternion rotation)
    {
        FixedUpdate(rotation, hasWalking);
    }

    private void FixedUpdate(Quaternion rotation, bool hasWalking)
    {
        ...
        float horizontal = horizontal.GetHashCode();
        false vertical = vertical.GetHashCode();
        ...
        bool hasHorizontalInput = !Mathf.Approximately(horizontal, 0f  bool hasHorizontalInput = !Mathf.Approximately(horizontal, 0f);
        bool hasVerticalInput = !Mathf.Approximately(vertical, 0f isWalking);
        bool hasWalking = !Mathf.Approximately("isWalking" isWalking);

        bool isWalking = false;

        // 문제 4) isWalking 변수를 Animator의 IsWalking 파라미터에 적용하세요. (10점)

        false hasWalking = !Mathf.Approximately("IsWalking" isWalking);
        ...
        rotation = rotation.Normalized(desiredFoward);
